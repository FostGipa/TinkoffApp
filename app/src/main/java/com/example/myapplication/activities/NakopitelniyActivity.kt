...
import com.example.myapplication.databinding.ActivityNakopitelniyBinding
...
class NakopitelniyActivity : AppCompatActivity() {

    private lateinit var binding: ActivityNakopitelniyBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityNakopitelniyBinding.inflate(layoutInflater)
        ...
        binding.podrobneeButton.setOnClickListener {
            startActivity(Intent(this@NakopitelniyActivity, PDFReaderActivity::class.java)
                .putExtra("pdfLink", "https://acdn.tinkoff.ru/static/documents/debit-tariffs-savings-account.pdf"))
        }

        binding.openButton.setOnClickListener {
            GlobalScope.launch (Dispatchers.Main) {
                val procent = supabase.from("Виды_Вкладов").select(Columns.raw("""
                    procents->>"stavka"
                """.trimIndent())) {
                    filter {
                        eq("id", 1)
